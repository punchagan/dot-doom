...
;; Journal files stay chronological (new entries at the bottom, matching
;; the day's narrative order). Everything else gets new items at the
;; top -- easier to spot what just landed. First matching regexp wins.
...
Uses `add-face-text-property' rather than `put-text-property' so this
background layers on top of whatever's already there (e.g. the boxed
todo-keyword face) instead of replacing it outright -- otherwise the
DRAFT keyword loses its box and looks like plain text."
    (save-excursion
      (goto-char (point-min))
      (while (not (eobp))
        (let* ((line-end (line-end-position))
          ...
          (when (equal (org-find-text-property-in-string 'todo-state line-text) "DRAFT")
            (add-face-text-property (line-beginning-position) (1+ line-end)
                                     'pc/agenda-draft-face)))
        (forward-line 1))))
  ...
  (defun pc/org-refile-subtree-to-tasks ()
    "Refile the current subtree to the top of tasks.org -- a flat
file, no heading structure (see the \"t\" capture template), so this
just pastes it in at level 1."
    (interactive)
    (let ((tasks-buf (find-file-noselect (expand-file-name "tasks.org" org-directory))))
      (org-cut-subtree)
      ;; Set the continuation position when this function is called from org-map-entries
      (setq org-map-continue-from (point))
      (save-mark-and-excursion
        (set-buffer tasks-buf)
        (goto-char (point-min))
        (org-paste-subtree 1)
        (save-buffer))))
...
capture timestamp."
  ...
  (defun pc/dispatch-todo ()
    ...
    (interactive)
    (org-todo "TODO")
    (pc/org-refile-subtree-to-tasks))

  (defun pc/dispatch-kill ()
    "Mark the entry KILL and refile it to tasks.org."
    (interactive)
    (org-todo "KILL")
    (pc/org-refile-subtree-to-tasks))
...
:CREATED: property dated today."
  ...
  (org-ql-defpred created-today ()
    "Return non-nil if entry's :CREATED: property is dated today.
Unlike the built-in `ts-inactive' predicate, this only looks at
:CREATED:, so it won't also match entries with some other inactive
timestamp today (e.g. a habit's completion logged in :LOGBOOK:)."
    :body
    (let ((created (org-entry-get (point) "CREATED")))
      (and created (string-match-p (regexp-quote (format-time-string "%Y-%m-%d")) created))))

  (defun pc/agenda-created-today ()
    "Show entries across `org-agenda-files' with a :CREATED: property
dated today -- the same entries counted by
`pc/entries-created-today-count'."
    (interactive)
...
(defun pc/habit-weekly-counts (start-day end-day)
  "Return an alist of (HEADING . COUNT) for every entry across
`org-agenda-files' with a non-nil :SHOW_IN_WEEKLY: property, COUNT
being how many times it was marked DONE between START-DAY and
END-DAY (inclusive).

Counts are read off the habit's own state-change log lines (e.g. -
State \"DONE\"       from \"\"           [2026-07-25 Sat 23:24] --
see recurring.org), not a :LOGBOOK: drawer -- these habits log
completions directly under the heading."
  (let (counts)
    (org-map-entries
     (lambda ()
       (when (org-entry-get nil "SHOW_IN_WEEKLY")
         (let ((heading (org-get-heading t t t t))
           ...
           (while (string-match
                   ...
                   body pos)
             (when (<= start-day
                       (time-to-days (org-time-string-to-time (match-string 1 body)))
                       end-day)
               (setq count (1+ count)))
             (setq pos (match-end 0)))
           (push (cons heading count) counts))))
     nil 'agenda)
    (nreverse counts)))

(defun pc/habit-weekly-report (start-day end-day)
  "Return an Org subtree (as text, a level-1 heading followed by a
plain list) summarizing every :SHOW_IN_WEEKLY: habit's count between
START-DAY and END-DAY -- for `pc/create-weekly' to paste in
alongside the week's collected entries. nil when nothing's tagged
:SHOW_IN_WEEKLY: yet, so there's nothing to paste."
  (let ((counts (pc/habit-weekly-counts start-day end-day)))
    (when counts
      (concat "* Habits\n"
              (mapconcat
               (lambda (c) (format "- %s: %d time%s" (car c) (cdr c)
                                    (if (= (cdr c) 1) "" "s")))
               counts "\n")
              "\n"))))
...
Also appends a \"Habits\" section (see `pc/habit-weekly-report')
tallying any :SHOW_IN_WEEKLY: habit's completions over the same
range -- easy, low-effort weekly filler (\"3 gym sessions this
week\") for whichever habits are worth surfacing that way. Skipped
entirely when nothing's tagged :SHOW_IN_WEEKLY: yet.
...
confirming."
  ...
  (let* ((start-day (time-to-days start))
    ...
    (let ((habits (pc/habit-weekly-report start-day end-day)))
      (when habits
        (org-paste-subtree 2 habits)))
